...
const ALGORITHM_COPY: Record<string, { title: string; subtitle: string; note: string }> = {
  popularity: {
    title: "Popularity",
    subtitle: "A reliable cold-start baseline",
    note: "Ranks movies by a weighted blend of average rating and rating volume. It is stable, explainable, and useful when user history is sparse."
  },
  usercf: {
    title: "UserCF",
  ...
  itemcf: {
    ...
    note: "Looks at the user's rated movies, then surfaces items that are similar in the collaborative rating matrix."
  },
  mf: {
    title: "MF / SVD",
  ...
  hybrid: {
    ...
    note: "Blends popularity, collaborative filtering, matrix factorization, and content signals to balance accuracy, diversity, and interpretability."
  }
};
...
function overlapWithHybrid(results: Record<string, MovieCard[]> | null, key: string) {
  ...
  return `${overlap} of ${ids.length} titles also appear in Hybrid, showing where this single signal agrees with the final blend.`;
}
    ...
    <section className="space-y-8">
        ...
        <div className="mt-3 grid gap-5 lg:grid-cols-[1fr_420px] lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-300">
              This page is designed for course defense: it shows what each algorithm optimizes for, where their recommendations overlap, and why Hybrid tends to feel more product-ready.
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-black/30 p-4 backdrop-blur">
            ...
            <button onClick={load} className="mt-4 w-full rounded-full bg-neon px-5 py-2.5 text-sm font-bold text-ink transition hover:bg-white">
              Run Comparison
            </button>
          </div>
        </div>
      ...
      <div className="rounded-[1.6rem] border border-white/10 bg-panel/80 p-5">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          ...
          <p className="text-xs text-slate-500">Static mode uses cached model lists; the controls document the intended ranking formula.</p>
        </div>
          ...
          {(Object.keys(weights) as WeightKey[]).map((k) => (
            <label key={k} className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-slate-300">
              <span className="flex items-center justify-between font-semibold text-white">
                {k.toUpperCase()}
                <span className="text-neon">{weights[k].toFixed(2)}</span>
              </span>
              <input
                type="range"
                min={0}
